 ...
 * 
 */
public class SnakeMind implements Mind, Serializable, Cloneable{
	...
	public class MindMap implements Serializable{
		...
		public int check(BattleMap battleMap, Point coord) {
			...
			for (int i = 0; i < mindmap.length; i++)
				for (int j = 0; j < mindmap.length; j++)
					map270[i][j] = map90[mindmap.length - 1 - i][mindmap.length - 1
							- j];
	...
	public Object clone() throws CloneNotSupportedException {
		SnakeMind m = new SnakeMind(this.sn);
		m.mm = this.mm.clone();
